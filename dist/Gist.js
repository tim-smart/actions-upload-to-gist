...
const make = tsplus_module_1.flatMap(tsplus_module_1.service(Github), github => tsplus_module_1.flatMap(tsplus_module_1.service(tsplus_module_3.Git.Git), git => tsplus_module_1.flatMap(tsplus_module_1.service(RunnerEnv), runner => tsplus_module_1.map(tsplus_module_1.service(tsplus_module_3.Fs.Fs), fs => {
    ...
    const createBlank = (isPublic = false) => tsplus_module_1.map(create({
        files: {
            ".EMPTY": {
    ...
    const clone = (id) => tsplus_module_1.flatMap(runner.mkTmpDir(id), dir => tsplus_module_1.map(git.run((_) => _.clone(`https://${tsplus_module_2.value(github.token)}@gist.github.com/${id}.git`, dir, { "--depth": 1 })), () => dir));
    const cloneAndAdd = (id, path) => tsplus_module_1.flatMap(clone(id), dir => tsplus_module_1.flatMap(fs.copyFileOrDir(path, dir), () => tsplus_module_1.map(git.run((_) => _.add(".").commit(`Add ${path}`).push("origin", "main")), () => void 0)));
    const createAndAdd = (path) => tsplus_module_1.flatMap(createBlank(), gist => tsplus_module_1.map(cloneAndAdd(gist.id, path), () => void 0));
